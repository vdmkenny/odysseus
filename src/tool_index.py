...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    "bash": "Run shell commands on the server. Install packages, git operations, builds, system info, process management. Prefer the dedicated tools when they fit: read_file/write_file/edit_file for files, grep/glob/ls for searching/listing -- use bash only for what those do not cover. Do not use for web lookup/search; use web_search or web_fetch when web tools are available.",
    "python": "Execute Python code for computation, data processing, math, scripting, and parsing. Not for writing code for the user. Prefer the dedicated file tools (read_file/write_file/edit_file/grep/glob/ls) for reading, writing, or searching files. Do not use for web lookup/search; use web_search or web_fetch when web tools are available.",
